...
function RevenueStatsQuarter() {
  ...
  const handleQuarterChange = async (event) => {
    ...
    try {
        const response = await fetch("http://127.0.0.1:8000/revenue-stats-quarter/", {
            method: "POST",
            headers: {
    ...
    return (
        <Container>
            <div>
                ...
                {error && <p>{error}</p>}
                {totalRevenue > 0 && (
                    <p>Tổng doanh thu quý {quarter}: {totalRevenue} VND</p>
                )}
                {items.length > 0 && (
                    <div>
...
export default RevenueStatsQuarter